...
\item Drugs are just ligand molecules that bind to proteins to cause a therapeutic effect.
\end{itemize}
...
\item The project aim: Reduce the drug discovery costs by eliminating bad leads.
\item Predict protein-ligand binding affinity using "In-Silico" methods. 
\end{itemize}
...
\item We make use of Binding site prediction algorithms (fpocket/dpocket) for getting pocket data.
\item The features contain information about the 3D structures in our data.
\end{itemize}
...
\begin{itemize}
\item \textbf{Output Correlation}: The input features that have the best \textit{Pearson} and \textit{Spearman} correlation with the output were selected.
...
\item \textbf{$R^2$ score} (\textit{Coefficient of determination}): . $R^2  \in (- \infty, 1.0]$ where $1.0$ is the best score. 
